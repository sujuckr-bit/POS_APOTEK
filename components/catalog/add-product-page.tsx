...
export default function AddProductPage() {
  ...
  useEffect(() => {
    ...
    const returnedUnit = searchParams.get('unit')
    if (returnedUnit) setUnit(returnedUnit)
    if (!encodedPackage || packageLoaded.current) return
    ...
    try {
      ...
      router.replace(`/katalog/tambah?unit=${encodeURIComponent(returnedUnit || unit)}`)
    } catch {
      router.replace(`/katalog/tambah?unit=${encodeURIComponent(returnedUnit || unit)}`)
    }
  }, [router, searchParams, unit])
    ...
    <main className="product-form-page">
      ...
      <form id="add-product-form" className="product-form" onSubmit={(event) => event.preventDefault()}>
        ...
        <section className="product-form-section">
          ...
          <button type="button" className="package-add" onClick={() => { const options = encodeURIComponent(JSON.stringify(packages.map(({ name, abbreviation, quantity, unit: packageUnit }) => ({ name, abbreviation, quantity: Number(quantity), unit: packageUnit })))); router.push(`/katalog/tambah/kemasan?unit=${encodeURIComponent(unit)}&options=${options}`) }} disabled={!unit}><Plus size={21} /> Satuan/kemasan</button>
        </section>
