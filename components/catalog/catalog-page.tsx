...
import { ArrowLeft, ChevronDown, Filter, Menu, Package, Search, SlidersHorizontal, UserRound } from 'lucide-react'
    ...
    <main className="pos-app catalog-app">
      <div className="page-area">
        <header className="page-header catalog-header">
          ...
          <div><span className="eyebrow">Inventaris apotek</span><h1>Katalog Produk</h1></div>
        ...
        <section className="catalog-page-content" aria-labelledby="catalog-title">
          <div className="catalog-intro"><div><span className="eyebrow">Daftar barang</span><h2 id="catalog-title">Produk aktif</h2><p>Kelola pandangan stok dan harga produk yang tersedia di apotek.</p></div><a className="catalog-pos-link" href="/"><Menu size={17} /> Buka kasir</a></div>
          ...
          <div className="catalog-product-grid">{products.map((product) => <article className="catalog-product-card" key={product.id}><div className="catalog-product-icon"><Package size={21} /></div><div className="catalog-product-main"><div className="catalog-product-top"><span className="catalog-code">{product.barcode}</span><span className={`status status-${product.status}`}>{statusLabel(product.status)}</span></div><h3>{product.name}</h3><p>{product.genericName} · {product.strength}</p><div className="catalog-product-meta"><span>Stok <strong>{product.availableStock} {product.unitLabel.toLowerCase()}</strong></span><strong>{formatRupiah(product.price)} <small>/ {product.unitLabel.toLowerCase()}</small></strong></div></div></article>)}{products.length === 0 && <div className="catalog-empty"><Search size={24} /><strong>Produk tidak ditemukan</strong><span>Coba kata kunci atau filter yang lain.</span></div>}</div>
        </section>
      </div>
    </main>
  )
