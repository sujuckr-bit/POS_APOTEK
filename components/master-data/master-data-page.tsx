...
export function MasterDataPage() {
  ...
  const saveProduct = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true)
    const { error } = await supabase.from('products').insert({ name: form.name.trim(), sku: form.sku.trim(), category_id: form.category_id, supplier_id: form.supplier_id || null, unit_id: form.unit_id, purchase_price: Math.max(0, Number(form.purchase_price) || 0), selling_price: Math.max(0, Number(form.selling_price) || 0), minimum_stock: minimumStock, stock: 0 })
    setSaving(false)
    if (error) {
  ...
  return <main className="master-page">
    <header className="master-header"><div><p className="eyebrow">Operasional apotek</p><h1>Data master</h1><p>Kelola produk, kategori, supplier, dan satuan penjualan dari satu tempat.</p></div><button className="master-primary" onClick={tab === 'Produk' ? () => { setFormError(''); setProductFormOpen(true) } : addReference}>Tambah {tab.toLowerCase()}</button></header>
    ...
    {isProductFormOpen && <div className="master-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setProductFormOpen(false)}><section className="master-modal" role="dialog" aria-modal="true" aria-labelledby="add-product-title"><div className="master-modal-head"><div><p className="eyebrow">Data master</p><h2 id="add-product-title">Tambah produk</h2></div><button type="button" className="master-modal-close" aria-label="Tutup form" onClick={() => setProductFormOpen(false)}>×</button></div><form onSubmit={saveProduct}><div className="master-form-grid"><label>Nama produk<input autoFocus value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Kode produk<input value={form.sku} onChange={(event) => setForm({ ...form, sku: event.target.value })} /></label><label>Kategori<select value={form.category_id} onChange={(event) => setForm({ ...form, category_id: event.target.value })}><option value="">Pilih kategori</option>{categories?.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Satuan dasar<select value={form.unit_id} onChange={(event) => setForm({ ...form, unit_id: event.target.value })}><option value="">Pilih satuan</option>{units?.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Supplier<select value={form.supplier_id} onChange={(event) => setForm({ ...form, supplier_id: event.target.value })}><option value="">Pilih supplier</option>{suppliers?.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Stok minimum<input type="number" min="0" value={form.minimum_stock} onChange={(event) => setForm({ ...form, minimum_stock: event.target.value })} /></label></div>{formError && <p className="master-form-error" role="alert">{formError}</p>}<div className="master-modal-actions"><button type="button" className="master-secondary" onClick={() => setProductFormOpen(false)}>Batal</button><button type="submit" className="master-primary" disabled={isSaving}>{isSaving ? 'Menyimpan…' : 'Simpan produk'}</button></div></form></section></div>}
  </main>
}
