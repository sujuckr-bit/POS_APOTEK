...
export function MasterDataPage() {
  ...
  return <main className={`master-page ${sidebarOpen ? 'sidebar-open' : ''} ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}><button className="master-sidebar-toggle" type="button" aria-label={sidebarOpen ? 'Sembunyikan menu data master' : 'Tampilkan menu data master'} aria-expanded={sidebarOpen} onClick={() => setSidebarOpen((open) => !open)}>☰</button><button className="master-sidebar-overlay" type="button" aria-label="Tutup menu data master" onClick={() => setSidebarOpen(false)} /><aside className="master-sidebar"><button type="button" className="master-sidebar-back" onClick={() => router.push('/')} aria-label="Kembali ke beranda"><span aria-hidden="true">←</span><span>Kembali</span></button><div className="master-sidebar-brand"><span>R</span><div><strong>Risyah</strong><small>Data Master</small></div><button className="master-sidebar-close" type="button" aria-label="Tutup menu data master" onClick={() => setSidebarOpen(false)}>×</button></div><div className="master-sidebar-desktop-toggle"><button type="button" aria-label={sidebarCollapsed ? 'Tampilkan sidebar' : 'Sembunyikan sidebar'} onClick={() => setSidebarCollapsed((collapsed) => !collapsed)}>{sidebarCollapsed ? '›' : '‹'}</button></div><p className="master-sidebar-label">Katalog</p><nav aria-label="Navigasi data master">{(['Produk', 'Kategori', 'Supplier', 'Satuan'] as Tab[]).map((item) => <button type="button" key={item} className={tab === item ? 'master-sidebar-item active' : 'master-sidebar-item'} onClick={() => setTab(item)}><span className="master-sidebar-icon" aria-hidden="true">{item === 'Produk' ? 'P' : item === 'Kategori' ? 'K' : item === 'Supplier' ? 'S' : 'U'}</span>{item}{tab === item && <span className="master-sidebar-dot" aria-hidden="true" />}</button>)}</nav><div className="master-sidebar-footer">Kelola referensi apotek<br /><strong>Rapi dan terkontrol</strong></div></aside><div className="master-content"><header className="master-header"><div><p className="eyebrow">Operasional apotek</p><h1>Data master</h1><p>Kelola produk, kategori, supplier, dan satuan penjualan dari satu tempat.</p></div><button className="master-primary" onClick={tab === 'Produk' ? () => router.push('/master-data/produk/tambah') : addReference}>Tambah {tab.toLowerCase()}</button></header>
