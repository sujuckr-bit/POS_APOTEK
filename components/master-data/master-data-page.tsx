...
export function MasterDataPage() {
  ...
  const addProduct = async () => {
    const { data: unit } = await supabase.from('units').select('id').eq('is_active', true).order('name').limit(1).maybeSingle()
    const sku = `NEW-${Date.now().toString().slice(-6)}`
    const { error } = await supabase.from('products').insert({ sku, name: 'Produk baru', unit_id: unit?.id ?? null, selling_price: 0, purchase_price: 0, stock: 0, minimum_stock: 0 })
    if (error) window.alert('Produk belum dapat ditambahkan. Periksa satuan aktif terlebih dahulu.')
    else await mutateProducts()
  }
  ...
  return <main className="master-page">
    <header className="master-header"><div><p className="eyebrow">Operasional apotek</p><h1>Data master</h1><p>Kelola produk, kategori, supplier, dan satuan penjualan dari satu tempat.</p></div><button className="master-primary" onClick={tab === 'Produk' ? addProduct : addReference}>Tambah {tab.toLowerCase()}</button></header>
