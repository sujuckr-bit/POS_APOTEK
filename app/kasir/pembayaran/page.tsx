...
export default function PaymentPage() {
  ...
  const updateUnit = (id: string, unitIndex: number) => setCart((lines) => lines.map((line) => { if (line.product.id !== id) return line; const unit = line.product.units?.[unitIndex] ?? line.unit; return { ...line, unit, quantity: Math.min(line.quantity, Math.max(1, Math.floor(line.product.availableStock / unit.conversionFactor))) } }))
  const pay = async () => {
